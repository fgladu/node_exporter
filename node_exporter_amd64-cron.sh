...
if [ -z "$INSTALLED_VERSION" ]; then
    echo "Failed to determine the currently installed version on $HOSTNAME."
    exit 1
fi
...
if [ -z "$LATEST_VERSION" ]; then
    echo "Failed to fetch the latest version from GitHub API on $HOSTNAME."
    exit 1
fi
...
for i in {1..3}; do
    ...
    if [[ $STATUS_OUTPUT =~ "Active: active" ]]; then
        echo "Service on $HOSTNAME updated and started successfully."
        ...
        echo "Failed to start service on $HOSTNAME (Attempt $i)."
    fi
done
