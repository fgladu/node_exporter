...
if [ "$INSTALLED_VERSION" == "$VERSION" ]; then
	echo "$HOSTNAME - node_exporter is already up-to-date (version $INSTALLED_VERSION). Exiting."
	exit 0
fi

echo "$HOSTNAME - Updating node_exporter from version $INSTALLED_VERSION to version $VERSION."
...
for i in {1..3}; do
	...
	if [[ $STATUS_OUTPUT =~ "Active: active" ]]; then
		...
		echo "$HOSTNAME - Failed to start service (Attempt $i)."
	fi
done
